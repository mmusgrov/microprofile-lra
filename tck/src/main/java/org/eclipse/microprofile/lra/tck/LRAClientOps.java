...
import static org.eclipse.microprofile.lra.tck.participant.api.NonParticipatingTckResource.STAUS_CODE_QUERY_NAME;
...
public class LRAClientOps implements Cloneable {
    /**
    ...
    /**
     * Key for looking up the config property th
     * recovery coordinator is running on
     */
    static final String LRA_RECOVERY_PATH_KEY = "lra.coordinator.recovery.path";
    ...
    private int getStatus(String lra) {
        ...
        if (status == 200) {
            ...
            try {
                ...
                fail("getStatus: invalid URI " + lra);
            }
        }
    ...
    private Response invokeRestEndpoint(String lra, String basePath, String path, int coerceResponse) {
        WebTarget resourcePath = target.path(basePath).path(path).queryParam(STAUS_CODE_QUERY_NAME, coerceResponse);
