...
:authors: Michael Musgrove, Nicola Ferraro, Ondra Chaloupka, Tom Jenkinson, MP google groups forum, ad hoc discussions ...
...
----
  ...
  @Path("/doitASAP")
  @Produces(MediaType.APPLICATION_JSON)
  @LRA(value = LRA.Type.REQUIRED, timeLimit = 100, timeUnit = TimeUnit.MILLISECONDS)
...
----
    @PUT
    @Path("/compensate")
    @Compensate(timeLimit = 100, timeUnit = TimeUnit.MILLISECONDS)
    public Response completeWork(@HeaderParam(LRA_HTTP_HEADER) String lraId,
                                 String userData) { ... }
----
...
(https://github.com/eclipse/microprofile-lra/tree/master/api/src/main/java/org/eclipse/microprofile/lra/participant[github link]).
This interface will be used during recovery to ask participants to complete or compensate.
...
----
@Path("/")
public class UnannotatedParticpant implements LRAParticipant, Serializable {
    @Inject
    ...
    public void doInTransaction() throws JoinLRAException
    {
        ...
        URL lraId = lraClient.startLRA(
                        "Unannotated client", 0L, TimeUnit.MILLISECONDS);
...
It is important to stress that participants must be serializable for this approach
...
definitions of <<source-LRAParticipant,LRAParticipants>>
(https://github.com/eclipse/microprofile-lra/blob/master/api/src/main/java/org/eclipse/microprofile/lra/participant/LRAParticipant.java[github link])
are made available to the recovery system
and that deserializers are registered as soon as the JVM hosting the original
participants is started. A typical implementation might, for example, register
