...
activity by annotating it with both the `@Compensate` and the JAX-RS
...
<<source-CompensatorStatus,CompensatorStatus enum values (as reported by the enum `name()` method)>>.
...
definitions of <<source-LRAParticipant,LRAParticipants>> are made available to the recovery system
