...
Note that the mechanics of canceling a nested LRA is no different from a nested LRA.
...
MUST make the context available either via an JAX-RS header param.
...
Although the programmatic API will not be available in the first release of this
specification there are scenarios where a business method may wish to make runtime decisions
...
public void bookTrip(@HeaderParam(LRA_HTTP_HEADER) URI lraId) {
...
@Compensate
@PUT
void cancelFlight(@HeaderParam(LRA_HTTP_HEADER) URI lraId) {...}
...
----
    ...
    public void bookTrip(@HeaderParam(LRA_HTTP_HEADER) URI lraId) {
        ...
        // by invoking a suitable JAX-RS resource method annotated
        // with @Nested and @LRA(value = LRA.Type.MANDATORY, end = false)
        flightLraId = ...
...
This JAX-RS invocation creates a new LRA nested under the one identified by `lraId`.
...
[source,java]
----
   // cancel the nested LRA by invoking a suitable JAX-RS resource method
   // annotated with, for example
   //  @LRA(value = LRA.Type.SUPPORTS, end = true)
   // In this example though the active context is not flightLraId so it needs
   // to be explicity set in the request headers:

        Response response = null;
        try {
            response = aResourcPathAnnotatedWithLRAEndEqualsTrue
                    .request()
                    .header(LRA.LRA_HTTP_HEADER, flightLraId)
                    .put(Entity.text(""));
            ...
        finally {
            response.close();
